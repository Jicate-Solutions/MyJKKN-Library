		...
		if (error) {
			...
			if (error.code === '23514') {
				return NextResponse.json(
					{ error: `The database does not allow the frequency "${body.frequency}" yet — run the pending database update (20260916_lib_subscription_frequency_five_yearly) and try again` },
					{ status: 400 }
				)
			}
