...
CREATE INDEX IF NOT EXISTS idx_student_backlogs_last_attempt_session_id ON student_backlogs (last_attempt_session_id);
CREATE INDEX IF NOT EXISTS idx_student_backlogs_original_academic_year_id ON student_backlogs (original_academic_year_id);
CREATE INDEX IF NOT EXISTS idx_student_backlogs_original_semester_result_id ON student_backlogs (original_semester_result_id);
...
-- Composite index for student_backlogs queries filtered by student + status
CREATE INDEX IF NOT EXISTS idx_student_backlogs_student_status ON student_backlogs (student_id, status);
...
CREATE INDEX IF NOT EXISTS idx_answer_sheet_packets_unassigned ON answer_sheet_packets (exam_timetable_id) WHERE evaluator_id IS NULL;

-- Partial index for open student backlogs
CREATE INDEX IF NOT EXISTS idx_student_backlogs_open ON student_backlogs (student_id, course_code) WHERE status = 'open';

-- ==============================================================================
-- Indexes for Common Search and Filter Patterns
-- ==============================================================================

-- Index for searching students by register number (used heavily in reports)
-- This may already exist from migration 20251103_add_index_student_register_number.sql
...
COMMENT ON INDEX idx_student_backlogs_open IS 'Partial index for open backlogs only';

-- ==============================================================================
-- Verification Query
-- ==============================================================================

-- Run this to verify all FK columns now have indexes:
-- SELECT
