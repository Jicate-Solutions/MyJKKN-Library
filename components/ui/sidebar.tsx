...
>(
  (
    ...
    // Always initialize with defaultOpen to match server render and avoid hydration mismatch.
    const [_open, _setOpen] = React.useState(defaultOpen)
    const open = openProp ?? _open

    // After mount, sync state from cookie/localStorage (client-only)
    React.useEffect(() => {
      // Try cookie first
      ...
      if (sidebarCookie) {
        ...
        const savedOpen = value === 'true'
        if (savedOpen !== _open) _setOpen(savedOpen)
        return
      }
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="group"
      className={cn("relative flex w-full min-w-0 flex-col p-2 pb-3", className)}
      {...props}
    />
  )
})
...
>(({ className, ...props }, ref) => (
  <ul
    ...
    className={cn(
      "mx-2 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border px-2 py-0.5",
      "group-data-[collapsible=icon]:hidden",
      className
    )}
    {...props}
